...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        current_child_text = ""
        for p in parent_paras:
            if len(current_child_text) + len(p) > child_size and current_child_text:
                children.append(Chunk(text=current_child_text.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                current_child_text = ""
            current_child_text += p + "\n\n"
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    if sections and sections[0].strip():
        chunks.append(Chunk(text=sections[0].strip(), metadata={**metadata, "section": "none", "strategy": "structure"}))
